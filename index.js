...
Usa ${prefix}verify para verificarte y poder usar el bot.`
				...
				num = anu.participants[0]
				teks = `Le hizo falta odio a @${num.split('@')[0]} bye Aborto ni c te ocurra volver 👋`
				Lxa.sendMessage(mdata.id, teks, MessageType.text, {contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
...
//---Metadata stiker
			function addMetadata(packname, author) {	
				if (!packname) packname = '😼𝗞𝗘𝗧𝗭𝗔 𝗕𝗢𝗧💨  Admin'; if (!author) author = 'Bot-ketzakot';	
...
//--Convert stiker to image
				case 'toimg':
					...
					exec(`ffmpeg -i ${medimg} ${ran}`, (err) => {
						...
						buffer = fs.readFileSync(ran)
						Lxa.sendMessage(from, buffer, image, {quoted: mek, caption: 'Ketza-Bot'})
						fs.unlinkSync(ran)
					})
					break
